            ...
            <div class="row">
              <div class="col-xs-6">
                <a href="/users/{{$user->id}}/edit" class="btn btn-small btn-info btn-block">
                  <i class="fa fa-pencil fa-fw" aria-hidden="true"></i> Editar<span class="hidden-xs hidden-sm"></span><span class="hidden-xs"> Usuario</span>
                </a>
              </div>
              {!! Form::open(array('url' => 'users/' . $user->id, 'class' => 'col-xs-6')) !!}
                {!! Form::hidden('_method', 'DELETE') !!}
                {!! Form::button('<i class="fa fa-trash-o fa-fw" aria-hidden="true"></i> Eliminar<span class="hidden-xs hidden-sm"></span><span class="hidden-xs"> Usuario</span>', array('class' => 'btn btn-danger btn-block btn-flat','type' => 'button', 'data-toggle' => 'modal', 'data-target' => '#confirmDelete', 'data-title' => 'Delete User', 'data-message' => 'Are you sure you want to delete this user ?')) !!}
              {!! Form::close() !!}
            </div>
          </div>
        </div>
